...
"""
# Find accounts with the trial expired and send an email reporting it
#
# python find_trial_expired.py ADMIN_PASSWORD
#
#
...
regexpStr = config.get('find_trial_expired', 'regexp')
...
template_subject_es = config.get('find_trial_expired', 'subject_es')
template_subject_en = config.get('find_trial_expired', 'subject_en')
template_file_es = config.get('find_trial_expired', 'file_es')
template_file_en = config.get('find_trial_expired', 'file_en')
...
folder_out = config.get('find_trial_expired','folder_out')
expired_days = config.get('find_trial_expired','expired_days')
marketing_email = config.get('find_trial_expired','email')
...
for billable_account in accounts:
    ...
    if len(promo)>0:
        toexpire_days = ((int(datetime.datetime.now().strftime('%s')) - promo[0]["timestamp"]) / 86400) - int(expired_days)
        ...
        if toexpire_days==0:
            ...
            with open('%s/trial_expired-%s.json' % (folder_out, time.strftime("%Y-%m-%d")), 'w') as outfile:
